  ...
  describe('searchImages()', () => {
    ...
    it('should return results', async () => {
      ...
      await expect(searchImages('minecraft', { vqd: SEARCH_QUERY_VQD })).to.eventually.have.nested.include({
        ...
        'results[0].image':
          'https://www.gamepur.com/files/images/2019-11/minecraft-earth-how-to-craft-and-smelt.jpg',
    ...
    it('should return results in strict safe search', async () => {
      ...
      await expect(
        searchImages('minecraft', { vqd: SEARCH_QUERY_VQD, safeSearch: SafeSearchType.STRICT })
      ).to.eventually.have.nested.include({
        noResults: false,
        vqd: SEARCH_QUERY_VQD,
    ...
    it('should throw on invalid options', async () => {
      // @ts-expect-error
      await expect(
        searchImages('a', { safeSearch: 'zzz' }),
        'invalid safe search was not caught'
      ).to.eventually.be.rejectedWith(TypeError, 'zzz is an invalid safe search type!');
      // @ts-expect-error
      await expect(
        searchImages('a', { offset: 'a' }),
        'invalid offset was not caught'
      ).to.eventually.be.rejectedWith(TypeError, 'Search offset is not a number!');
      await expect(
        searchImages('a', { offset: -10 }),
        'negative offset was not caught'
      ).to.eventually.be.rejectedWith(RangeError, 'Search offset cannot be below zero!');
      // @ts-expect-error
      await expect(
        searchImages('a', { locale: 1 }),
        'invalid lcoale was not caught'
      ).to.eventually.be.rejectedWith(TypeError, 'Search locale must be a string!');
      // @ts-expect-error
      await expect(
        searchImages('a', { size: 'zzz' }),
        'invalid image size was not caught'
      ).to.eventually.be.rejectedWith(TypeError, 'zzz is an invalid image size filter!');
      // @ts-expect-error
      await expect(
        searchImages('a', { type: 'zzz' }),
        'invalid image type was not caught'
      ).to.eventually.be.rejectedWith(TypeError, 'zzz is an invalid image type filter!');
      // @ts-expect-error
      await expect(
        searchImages('a', { layout: 'zzz' }),
        'invalid image layout was not caught'
      ).to.eventually.be.rejectedWith(TypeError, 'zzz is an invalid image layout filter!');
      // @ts-expect-error
      await expect(
        searchImages('a', { color: 'zzz' }),
        'invalid color was not caught'
      ).to.eventually.be.rejectedWith(TypeError, 'zzz is an invalid color filter!');
      // @ts-expect-error
      await expect(
        searchImages('a', { license: 'zzz' }),
        'invalid license was not caught'
      ).to.eventually.be.rejectedWith(TypeError, 'zzz is an invalid license filter!');
      await expect(
        searchImages('node', { vqd: 'zzz' }),
        'invalid vqd was not caught'
      ).to.eventually.be.rejectedWith(Error, 'zzz is an invalid VQD!');
    });

    it('should throw on server errors', async () => {
      const scope = makeSearchErrorNock('/i.js', DEFAULT_QUERY);

      await expect(searchImages('minecraft', { vqd: SEARCH_QUERY_VQD })).to.eventually.be.rejectedWith(
        Error,
